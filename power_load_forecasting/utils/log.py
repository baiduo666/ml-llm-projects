...
class Logger(object):
    ...
    def __init__(self, root_path, log_name, level='info', fmt='%(asctime)s - %(levelname)s: %(message)s'):
        ...
        # 设置日志级别
        level_value = self.level_relations.get(level)
        if level_value is None:
            raise ValueError(f"未知的日志级别: {level!r}，请选择: {list(self.level_relations.keys())}")
        self.logger.setLevel(level_value)

    def get_logger(self):
        # 如果 logger 已有 handler，直接返回，避免重复添加
        if self.logger.handlers:
            return self.logger

        # 指定对应的 Handler 为 FileHandler 对象
